...
public static class EditorMain
{
    ...
    public static void Draw(SpriteBatch spriteBatch)
    {
        ...
        for (int i = 0; i < Lines.Count; i++)
        {
            ...
            string[] words = Regex.Matches(Lines[i], @"//[^\n]*|\s+|'[^']*'|""[^""]*""|[^\w\s]|[\p{L}\p{N}_]+").Select(m => m.Value).ToArray();
            ...
            for (int j = 0; j < words.Length; j++)
            {
                ...
                string lookupString = words[j];
                
                if (lookupString.Length > 1)
                {
                    if (lookupString[0] == '/' && lookupString[1] == '/') lookupString = "//";
                }

                TextToColor.Lookup.TryGetValue(lookupString, out Color color);
